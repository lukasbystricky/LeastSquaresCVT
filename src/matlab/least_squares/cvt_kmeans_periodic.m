function [generators,energy, masses, areas] = cvt_kmeans_periodic(generators0, Phi, ...
...
for iter = 1 : iterations
    % scatter(generators(:,1, iter), generators(:,2, iter))
    ...
    % Create vector for virtual points, and rescale gens and samples to 
    %   [0,1]^n
    v_sample = ( sample + 1 ) / 2;
    generators(:,:,iter) = ( generators(:,:,iter) + 1 ) / 2;
    
    
    k = zeros(num_sample, 1);
    dist = zeros(num_sample, 1)+100;
    
    % Very naive distance function, loop over all sample points 
    %   and find closest generator point from there
    for i = 1 : num_sample
        for j = 1 : n_generators
            y = generators(j, :, iter);
            x = v_sample(i, :);
            
            % Calculate closest periodic distance and in which 
            %   direction virtual point is found
            [v_dist, v_ind] = min( abs([ x - y - ones(1,d) ; ...
                                         x - y ; ...
                                         x - y + ones(1, d) ]));
            v_dist = sum( v_dist.^2, 2);
            if v_dist < dist(i)
                v_pt = x + ones(1,d).*( v_ind - 2 );
                v_sample(i, :) = v_pt; % Replace in array with actual point
                dist(i) = v_dist;
                k(i) = j;
            end
        end
    ...
    % Rescale generators back to [-1,1]^n
    generators(:,:,iter) = 2 * generators(:,:,iter) - 1;
    ...
    for i = 1 : n_generators        
        ...
        % Use virtual point to compute centroid
        bins(i,:) = sum(bsxfun(@times, v_sample(k == i,:), weights(k == i)),1);
        bin_count(i) = sum(weights(k == i));
        
        % Take mod of virtual points to get correct weight
        masses(i, iter) = sum(Pweight(mod(v_sample(k == i,:), 1)))/ num_sample;       
    ...
    generators(:,:,iter + 1) = 2 * mod( generators(:,:,iter + 1), 1) - 1;
